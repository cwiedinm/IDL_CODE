...
; GET INPUT AND OUTPUT FILES
 ; Set file names and pathways
   lctfile = 'lct_ca.txt'
   treefile = 'pcttree_ca.txt'
   herbfile = 'pctherb_ca.txt'
   barefile = 'pctbare_ca.txt'
   firefile = 'fire_ca.txt'
...
; Set up output file (comma delimited text file)
	...
	printf, 6, 'xvalue, yvalue, lct,lcta1,lctm1,percentTree,percentherb,percentbare,CO'
	form = '(E20.9,",",E20.9,",",(3(I5,",")),E20.10,",",E20.10,",",E20.10,",",E25.10)'
; ******************************************************************************************
;
...
; Calculate/Estimate biomass burned- to be changed and incresed later!
			...
			If lct[j] eq 13 then begin
				if lct[j-1] eq 13 then begin
					if lct[j+1] eq 13 then begin
						print, 'All surrounding cells are urban!: ', lct[j], lct[j+1], lct[j-1]
 						goto, quitearly
 					endif else begin
			...
			;printf, 6, 'xvalue, yvalue, lct,lct+1,lct-1,percentTree,percentherb,percentbare,CO'
       		printf,6,format=form,xvalue,yvalue,lct[j],lct[j+1],lct[j-1],pcttree[j], pctherb[j],pctbare[j],emisco[j]
       	    LOOP:
 	endfor
endfor
