...
; Open the files that are needed for the modeling
	...
	printf, 6, 'xvalue, yvalue, lct,lctm,percentTree,percentherb,percentbare,CO2,CO,PM10,PM25,NOx,NH3,SO2,VOC'
	form = '(E20.9,",",E20.9,",",(2(I5,",")),E20.10,",",E20.10,",",E20.10,",",8(E25.10,","))'
; ******************************************************************************************
;
...
; Process only those grids with fires identified
 		if (fireloc[j,i] lt 8) then goto, LOOP ;This identifies the grids with fires (value 8 or 9)
;***************************************************************
...
; Estimate Fire Area
			...
			; April 1, 2004: This section was edited to account for fires that occur on the edges of the domain
			...
			If (lct[j,i] eq 0) or (lct[j,i] eq 15) or (lct[j,i] eq 255) or (lct[j,i] eq 254) then begin
 				...
 				print, '		**The lct[j+1,i] =',lct[j+1,i], ' and lct[j-1,i]= ', lct[j-1,i]
			...
			If lct[j,i] eq 13 then begin
				if lct[j-1,i] eq 13 then begin
					if lct[j+1,i] eq 13 then begin
						print, 'All surrounding cells are urban! j =', j,lct[j,i], lct[j+1,i], lct[j-1,i]
			...
			calculate:
				; CALCULATE EMISSIONS
				; CO2 Emissions
					emisco2[j,i] = area*((pcttree[m,i]/100.*p*pctburn[0,k,lct[m,i]]*bd[0,lct[m,i]]*emisfac[0,0,lct[m,i]])+(pctherb[m,i]/100.*p*pctburn[1,k,lct[m,i]]*bd[1,lct[m,i]]*emisfac[1,0,lct[m,i]]))
				; CO Emissions
					emisco[j,i] = area*((pcttree[m,i]/100.*p*pctburn[0,k,lct[m,i]]*bd[0,lct[m,i]]*emisfac[0,1,lct[m,i]])+(pctherb[m,i]/100.*p*pctburn[1,k,lct[m,i]]*bd[1,lct[m,i]]*emisfac[1,1,lct[m,i]]))
				; PM10 Emissions
					emispm10[j,i] = area*((pcttree[m,i]/100.*p*pctburn[0,k,lct[m,i]]*bd[0,lct[m,i]]*emisfac[0,2,lct[m,i]])+(pctherb[m,i]/100.*p*pctburn[1,k,lct[m,i]]*bd[1,lct[m,i]]*emisfac[1,2,lct[m,i]]))
				; PM2.5 Emissions
					emispm25[j,i] = area*((pcttree[m,i]/100.*p*pctburn[0,k,lct[m,i]]*bd[0,lct[m,i]]*emisfac[0,3,lct[m,i]])+(pctherb[m,i]/100.*p*pctburn[1,k,lct[m,i]]*bd[1,lct[m,i]]*emisfac[1,3,lct[m,i]]))
				; NOx Emissions
					emisnox[j,i] = area*((pcttree[m,i]/100.*p*pctburn[0,k,lct[m,i]]*bd[0,lct[m,i]]*emisfac[0,4,lct[m,i]])+(pctherb[m,i]/100.*p*pctburn[1,k,lct[m,i]]*bd[1,lct[m,i]]*emisfac[1,4,lct[m,i]]))
				; NH3 Emissions
					emisnh3[j,i] = area*((pcttree[m,i]/100.*p*pctburn[0,k,lct[m,i]]*bd[0,lct[m,i]]*emisfac[0,5,lct[m,i]])+(pctherb[m,i]/100.*p*pctburn[1,k,lct[m,i]]*bd[1,lct[m,i]]*emisfac[1,5,lct[m,i]]))
				; SO2 Emissions
					emisso2[j,i] = area*((pcttree[m,i]/100.*p*pctburn[0,k,lct[m,i]]*bd[0,lct[m,i]]*emisfac[0,6,lct[m,i]])+(pctherb[m,i]/100.*p*pctburn[1,k,lct[m,i]]*bd[1,lct[m,i]]*emisfac[1,6,lct[m,i]]))
				; VOC Emissions
					emisvoc[j,i] = area*((pcttree[m,i]/100.*p*pctburn[0,k,lct[m,i]]*bd[0,lct[m,i]]*emisfac[0,7,lct[m,i]])+(pctherb[m,i]/100.*p*pctburn[1,k,lct[m,i]]*bd[1,lct[m,i]]*emisfac[1,7,lct[m,i]]))
			...
			if j eq 0 or i eq 0 then goto, print2
       		printf,6,format=form,xvalue,yvalue,lct[j,i],lct[m,i],pcttree[j,i], pctherb[j,i],pctbare[j,i],emisco2[j,i],emisco[j,i],emispm10[j,i],emispm25[j,i],emisnox[j,i],emisnh3[j,i],emisso2[j,i],emisvoc[j,i]
			print2:
			printf,6,format=form,xvalue,yvalue,lct[j,i],lct[m,i],pcttree[j,i], pctherb[j,i],pctbare[j,i],emisco2[j,i],emisco[j,i],emispm10[j,i],emispm25[j,i],emisnox[j,i],emisnh3[j,i],emisso2[j,i],emisvoc[j,i]
       	    LOOP:
 	endfor
