...
; read, "What extension do you want to use with the output files?", ext
    ext = 'CHECK_2006_OLD_100206_input'
...
;  read, "What fuel loads do you want to use? (ave = 1, low = 2, high = 3):", loadnum
       loadnum = 3 ; made this standard on April 04, 2007
;****************************
...
;     infile = pathy+'EPA_EI\Brad_files\2006_RSCA_01102007\MODIS_na_2006_modelin.txt' ; emission calculation 01/12/2006 (including HCN)
...
; ******WARNING****************** (discovered Jan. 29, 2007- need to rerun after this date)
; NOTE -- when running 2002- herb AND tree SHOULD BE SWITCHED!!! (SEE LINES BELOW TO CHANGE READING IN OF THE FILE).
; ******************************************************************
...
; OUTPUT file pathway
    ...
    path2 = 'D:\Data2\wildfire\EPA_EI\MODEL\OUTPUT\Output_APR07\'
...
; Input GLC emission factors (kg/Mg)
    ; Redid the emission factors on 09/01/05
        ...
        ;glcfactor = path4+'Orig_files\DEfault\New_glc_EFs_jan2007b_Hg.csv'    ; now includes HCN
...
; REdo Fields because now can export information directly from GIS (08/24/05)
    ...
    ; JANUARY 24, 2007: See header information for input files and change
    ; accordingly
    ; 1. LAT
    ; 2. Long
    ; 3. SPIX
    ...
    ; 8. pct_herb
    ; 9.. pct_tree
    ; 11: Country Name  *** CHECK THESE LAST ONES!
    ; 10: State Name
