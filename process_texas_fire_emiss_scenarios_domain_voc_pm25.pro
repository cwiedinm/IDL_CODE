...
;  - Edited to output VOC and PM2.5
; 
;-------------------------------
;+
...
pro process_TEXAS_FIRE_EMISS_SCENARIOS_domain_VOC_PM25
t0 = systime(1) ;Procedure start time in seconds
close, /all

year = 2008

for j = 6,6 do begin ; do same processing for every file
  close, /all
  inpath = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_OUTPUT\'
  if j eq 0 then begin ; 
      infile = inpath + 'FINNv1_TX_default_06252013.txt'
      output1 = inpath + 'FINNv1_TX_default_06252013_SUMMARY_VOC_PM25_area.txt'
  endif
  ...
  if j eq 6 then begin ; 
      ...
      output1 = inpath + 'SMARTFIRE_default2008_SUMMARY_voc_pm25_area.txt'
  endif
    
openw, 1, output1

printf, 1, 'MOnthly Total Emissions from '+infile
printf, 1, 'state, month, npoints,CO, NO, NOX, NO2, PM25,VOC, area'
form = '(A25,",",I4,",",I8,7(",",D25.10))'
...
for i = 0,11 do begin
        month = i+1
         ...
         for k = 0,nstates-1 do begin  ; Do loop over state names
              ...
              areatotal = total(area[monthnow[firenow]])
                ; printf, 1, 'state, month, npoints,CO, NO, NOX, NO2, PM25,VOC'
                  ;form = '(A25,",",I4,",",I8,3(",",D25.10))' 
            printf, 1, format = form, statename[k] ,month,n_elements(firenow),COtotal,NOtotal, NOxtotal, NO2total, PMtotal,VOCtotal,areatotal
            skip21:
