...
; Set up output file with NFDRS- Calculated Emissions
    if droughtnum eq 1 then outfile2 = 'Modelout'+year+'_NFDRS_withD_'+ext+'.txt'
    if droughtnum eq 2 then outfile2 = 'Modelout'+year+'_NFDRS_noD_'+ext+'.txt'
     openw, 7, path2+outfile2
     print, 'opened ', path2+outfile2
...
; Set up output file with FCCS- Calculated Emissions
     outfile_fccs = 'Modelout'+year+'_FCCS_'+ext+'.txt'
     openw, 11, path2+outfile_fccs
     print, 'opened ', path2+outfile_fccs
     form4 = '(I8,",",D20.10,",",D20.10,",",(4(I10,",")),11(D25.10,","),2(A15,","))'
     printf, 11, 'j,long,lat,day,glc,id,FCCS,area,bmass_fccs,CO2,CO,PM10,PM25,NOx,NH3,VOC,CH4,SO2,country,state'

; Set up output file with MODIS-Calculated Emissions
      outfile_modis = 'Modelout'+year+'_MODIS_'+ext+'.txt'
      openw, 12, path2+outfile_modis
      print, 'opened ', path2+outfile_modis
...
; fuel loads are kg/m2
    ; New nfdrs fuel loadings 09/01/05 with correction units
    nfdrsfuelload = path4+'Orig_files\NFDRS\NFRDS_fuelload.csv'
...
; fuel loadings for FCCS
    fccsfuel1 = path4+'\Orig_files\FCCS\fccs_modelin.txt'
        inloadfccsfuel=ascii_template(fccsfuel1)
        fccsfuel=read_ascii(fccsfuel1, template=inloadfccsfuel)
       ; Field1= FCCS ID
       ; Field2 = 1hr
       ; Field3 = 10hr
       ; Field4 = 100hr
...
; REdo Fields because now can export information directly from GIS (08/24/05)
    ...
    ;    Units for the NOAA layers: 10 g/m2
    ; 20: Country Name
    ; 21: State Name
...
; SET UP ARRAYS
    ...
    canfuel = map.field11
    fccs = map.field12
    ; Convert units of NOAA fuel inputs to from 10g/m2 to kg/m2
    noaa1 = map.field13*10./1000. ; tree leaf biomass
    noaa2 = map.field14*10./1000. ; tree branch (wood and bark)
    noaa3 = map.field15*10./1000. ; above ground tree mass
    noaa4 = map.field16*10./1000. ; total shrub mass
    noaa5 = map.field17*10./1000. ; grass mass
    noaa6 = map.field18*10./1000. ; litter
    noaa7 = map.field19*10./1000. ; coarse woody debris
    cntry = map.field20
    state = map.field21
...
if droughtnum eq 1 then printf, 9, 'This run did include drought for the NFDRS conditions'
if droughtnum eq 2 then printf, 9, 'This run did NOT include drought for the NFDRS conditions'
