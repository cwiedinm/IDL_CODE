...
;
;OCTOBER 29, 2016
;- edited to output 2010 fire emissions in aFRICA by genveg type
;- For Katie D's proposal
;-------------------------------
;+
...
pro clip_finnv15_TOTALS_AFRICA
...
print, 'J = ', j
  ...
  if j eq 8 then begin ; 2010
    year = 2010
     infile = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\OUTPUTS2014\JUNE2014\GLOBAL_FINNv15_2010_06262014.txt'
      output1 = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\OUTPUTS_2016\Africa_Output_FINNv15_2010_ag.txt'
      output2 = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\OUTPUTS_2016\Africa_Output_FINNv15_2010_nonag.txt'
  endif
  ...
  ; Latest updated 2014 file form December 2015
...
openw, 1, output1
printf, 1, 'genveg, lat,long,PM2.5,CO'

openw, 2, output2
printf, 2, 'genveg, lat, long, PM2.5, CO'
form = '(I6,4(",",D25.10))'
...
;  if (lati[i] lt latmax and lati[i] gt latmin and longi[i] gt lonmin and longi[i] lt lonmax and day[i] gt daymin and day[i] lt daymax) then begin
   if (lati[i] lt latmax and lati[i] gt latmin and longi[i] gt lonmin and longi[i] lt lonmax and genveg[i] eq 9 and day[i] gt daymin and day[i] lt daymax) then begin
    ; 'day,vegtype, latitude, longitude, area, CO,NOx,NMOC,NH3,SO2,PM2.5,PM10, OC, BC'
   printf, 1, format = form, genveg[i],lati[i], longi[i], pm25[i], co[i]
   endif
    if (lati[i] lt latmax and lati[i] gt latmin and longi[i] gt lonmin and longi[i] lt lonmax and genveg[i] ne 9 and day[i] gt daymin and day[i] lt daymax) then begin
    printf, 2, format = form, genveg[i],lati[i], longi[i], pm25[i], co[i]
    endif
endfor
