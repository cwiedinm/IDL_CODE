...
;
;October 07, 2013
;- Edited to grid on a 1 degree grid and output only PM species that are requested by Dom's student, Carly Reddington
;
;NOVEMBER 06, 2013
;- edited to output a region for Will Porter 
;
;-------------------------------
;+
...
pro CLIP_global_dailyemis_global_WILLPorter
...
if year eq 2004 or year eq 2008 or year eq 2012 then begin
  daymax = 366
endif else begin 
  daymax = 365
endelse
...
for i = 0,nfires-1 do begin
      ; printf, 1, 'latitude,Longitude,day,CO,VOC,NOx,PM2.5'
if (lati[i] gt latmin and lati[i] lt latmax and longi[i] gt lonmin and longi[i] lt lonmax and day[i] le daymax) then begin
      printf, 1, format = form, lati[i], longi[i],day[i],OC[i],BC[i],SO2[i],PM25[i]
endif
endfor  
