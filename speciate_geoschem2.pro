...
pro speciate_geoschem2
close, /all

;****************************************************************************************************
; CONVERSION FACTOR TABLE FOR VOCs
      convert ='E:\Data2\wildfire\EMISSIONS_PAPER2009\SPECIATION\GEOS-chem_VOC_speciate_12242012.csv'
...
infile = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\NOVEMBER2012\GLOBAL_JAN-NOV2012_11082012.txt'
outfile = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\JANUARY2013\JENNY\GLOB2012_GEOSCHEM_01142013.txt'
checkfile = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\JANUARY2013\JENNY\LOG_GLOB2012_GEOSCHEM_01142013.txt'

; Edited the output file on 02/23/2009
; edited 11/18/2009; added NO and NO2
openw, 5, outfile
printf, 5, 'DAY,TIME,GENVEG,LATI,LONGI,AREA,CO2,CO,NO,NO2,SO2,NH3,CH4,ACET,ACTA,ALD2,ALK4,BENZ,C2H2,C2H4,C2H6,C3H8,CH2O,DMS,GLYC,HAC,HCN,HCOOH,ISOP,MACR,MEK,MGLY,MONX,MVK,PRPE,TOLU,OC,BC,PM25'
form='(I6,",",I6,",",I6,36(",",D20.10))'
...
; Speciate VOC emissions. VOC is in kg and the output of this is mole GEOS-CHEM Emissions species
	         ...
	         ACTAemis[i] = VOC[i]*convert2GEOCH[1]
           ALD2emis[i] = VOC[i]*convert2GEOCH[2]
           ALK4emis[i] = VOC[i]*convert2GEOCH[3]
           BENZemis[i] = VOC[i]*convert2GEOCH[4]
           C2H2emis[i] = VOC[i]*convert2GEOCH[5]
           C2H4emis[i] = VOC[i]*convert2GEOCH[6]
           C2H6emis[i] = VOC[i]*convert2GEOCH[7]
           C3H8emis[i] = VOC[i]*convert2GEOCH[8]
           CH2Oemis[i] = VOC[i]*convert2GEOCH[9]
           DMSemis[i] = VOC[i]*convert2GEOCH[10]
           GLYCemis[i] = VOC[i]*convert2GEOCH[11]
           HACemis[i] = VOC[i]*convert2GEOCH[12]
           HCNemis[i] = VOC[i]*convert2GEOCH[13]
           HCOOHemis[i] = VOC[i]*convert2GEOCH[14]
           ISOPemis[i] = VOC[i]*convert2GEOCH[15]
           MACRemis[i] = VOC[i]*convert2GEOCH[16]
           MEKemis[i] = VOC[i]*convert2GEOCH[17]
           MGLYemis[i] = VOC[i]*convert2GEOCH[18]
           MONXemis[i] = VOC[i]*convert2GEOCH[19]
           MVKemis[i] = VOC[i]*convert2GEOCH[20]
           NOemis[i] = VOC[i]*convert2GEOCH[21]+NOemis[i]
           PRPEemis[i] = VOC[i]*convert2GEOCH[22]
           TOLUemis[i] = VOC[i]*convert2GEOCH[23]
...
;  ACET,ACTA,ALD2,ALK4,BENZ,C2H2,C2H4,C2H6,C3H8,CH2O,DMS,GLYC,HAC,HCN,HCOOH,ISOP,MACR,MEK,MGLY,MONX,MVK,PRPE,TOLU,OC,BC,PM25
...
; ACET,       ACTA,       ALD2,       ALK4,       BENZ,       C2H2,       C2H4,       C2H6,       C3H8,       CH2O,       DMS,
	ACETemis[i],ACTAemis[i],ALD2emis[i],ALK4emis[i],BENZemis[i],C2H2emis[i],C2H4emis[i],C2H6emis[i],C3H8emis[i],CH2Oemis[i],DMSemis[i],$
; GLYC,       HAC,       HCN,       HCOOH,       ISOP,       MACR,       MEK,       MGLY,       MONX,       MVK,       PRPE,TOLU,OC,BC,PM25	
	GLYCemis[i],HACemis[i],HCNemis[i],HCOOHemis[i],ISOPemis[i],MACRemis[i],MEKemis[i],MGLYemis[i],MONXemis[i],MVKemis[i],PRPEemis[i],$
; TOLU,       OC,       BC,       PM25 
  TOLUemis[i],OCemis[i],BCemis[i],PM25emis[i] 
