...
; Developed by Christine Wiedinmyer, December 30, 20conf05
;
...
pro global_fire_fccs_SMARTFIRE_texas
...
; OUTPUT FILE
; ****************************************************************************
        outfile = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_OUTPUT\SMARTFIRE_2008_FCCS.txt'
     openw, 6, outfile
     print, 'opened output file: ', outfile
     printf, 6, 'longi,lat,day,TIME,fccs,lct,genLC,pcttree,pctherb,area,bmass,CO2,CO,CH4,H2,NOx,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,FACTOR,STATE'
     form = '(D20.10,",",D20.10,",",(5(I10,",")),4(D20.10,","),17(D25.10,","),F6.3,",", A25)'
 
    ; Print an output log file
    logfile = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_OUTPUT\LOG_SMARTFIRE_2008_FCCS.txt'
       openw, 9, logfile
...
; ****************************************************************************
  ...
  infile = 'E:\Data2\wildfire\TEXAS\FIRE_2008\SMARTFIRE\SMARTFIRE_INPUT_FILE.csv'  
...
; latitude  longitude date_time area  type  LCT TREE  HERB  BARE  Region_num  FCCS  ADMIN_NAME
...
conf[*] = 100 
  ...
  goodfires= where(conf gt -1 and fccs ge 0)
...
;    day = fix(parts[2])
;    month = fix(parts[1])
     MMDD = Date2[i] - 20080000 ; ******* NEeD TO CHANGE HER
     month = fix(MMDD/100)
     day = (MMDD-(month*100))
...
; Emissions = area*BE*BMASS*EF
    ; Convert units to consistent units
       area= area4[j]*0.00404686 ; convert acres to km2
...
printf,6,format = form, lon[j],lat[j],jd[j],time[j],fccs[j],lct[j],genveg,tree[j],herb[j], $
       area,bmass,CO2,CO,CH4,H2,NOX,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,Factortrop[j],state[j]
...
printf, 9, 'the total number of fires removed with confidence < 20:', numorig-nconfgt20
printf, 9, ''
