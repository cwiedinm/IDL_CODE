...
; APRIL 13, 2011
;   - Added Hg to this versoin - renamed from global_fire.pro to global_fire_Hg.pro
;   - Edited outputs to have fewer variables in output file
;   - Added the time zone in input file to account for overpass time. 
; *********************************************************************************************
...
;   READ IN EMISSION FACTOR FILE
 ...
 ;   emisin = 'F:\Data2\wildfire\GLOBAL_MOZART\MODEL\INPUT\Model_Inputs\FINAL_EFS_08182010.csv' ; New file created 08/18/2010 (same as above, now includes PM10)
    emisin = 'F:\Data2\wildfire\GLOBAL_MOZART\MODEL\INPUT\Model_Inputs\FINAL_EFS_Hg_04132011.csv' ; New file created 04/13/2011 to include Hg
...
; LCT  GenVegType  GenVegDescript  CO2 CO  CH4 NMOC  H2  NOXasNO SO2 PM25  TPM TPC OC  BC  NH3 NO  NO2 NMHC
; LCT  GenVegType  GenVegDescript  CO2 CO  CH4 NMOC  H2  NOXasNO SO2 PM25  TPM TPC OC  BC  NH3 NO  NO2 NMHC
...
;
       ...
       HgEF = emis.field21      ; Hg emission factor (added 04/13/2011)
...
; OUTPUT FILE
; ****************************************************************************
;          outfile = 'E:\Data2\wildfire\Mercury\BEN_deFOY\MODEL_OUTPUT\NA2005_Hg_04132011c.txt' ; Ran 04/04/2011 ("c" at end of the file includes the time zone correction)
;          outfile = 'E:\Data2\wildfire\Mercury\BEN_deFOY\MODEL_OUTPUT\NA2004_Hg_04132011c.txt' ; Ran 04/04/2011
           outfile = 'E:\Data2\wildfire\Mercury\BEN_deFOY\MODEL_OUTPUT\NA2003_Hg_04132011c.txt' ; Ran 04/04/2011
                          
     openw, 6, outfile
     print, 'opened output file: ', outfile
     ; New output file just for BEN 
      printf, 6, 'longi,lat,day,genLC,CO2,CO,CH4,H2,NOx,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,Hg'
      form = '(D20.10,",",D20.10,",",(2(I10,",")),18(D25.10,","))'
; OLD FORMAT
;      printf, 6, 'longi,lat,day,TIME,lct,genLC,globreg,pct_tree,pct_herb,pct_bare,area,bmass,CO2,CO,CH4,H2,NOx,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,Hg,FACTOR'
;     form = '(D20.10,",",D20.10,",",(8(I10,",")),2(D20.10,","),18(D25.10,","),F6.3)'

    ; Print an output log file
    logfile = 'E:\Data2\wildfire\Mercury\BEN_deFOY\MODEL_OUTPUT\LOG_NA2003_Hg_04132011c.txt'
       openw, 9, logfile
...
; ****************************************************************************
    ; path for input file
  infile = 'E:\Data2\wildfire\Mercury\BEN_deFOY\MODIS_Fire_points\RSAC_MODIS2003\RSAC2003_NA_inputfile_format_TZ.csv' ; Created and input 04/13/2011
;  infile = 'E:\Data2\wildfire\Mercury\BEN_deFOY\MODIS_Fire_points\RSAC_MODIS2004\RSAC2004_NA_inputfile_format_TZ.csv' ; Created and input 04/13/2011
;  infile = 'E:\Data2\wildfire\Mercury\BEN_deFOY\MODIS_Fire_points\RSAC_MODIS2005\RSAC2005_NA_inputfile_TZ_format.csv' ; Created and input 04/13/2011
; Read in FIRE FILE
...
; WGS84LAT  WGS84LONG SPIX  TPIX  DATE_ UTC CONFIDENCE  LCT TREE  HERB  BARE  Region_num  ZONE
; WGS84LAT  WGS84LONG SPIX  TPIX  DATE_ UTC CONFIDENCE  LCT TREE  HERB  BARE  Region_num  ZONE_
	...
	if yearnum eq 2008 or yearnum eq 2004 then begin
        ; set julian date (leap year)
         ...
         ntotdays = 366
          ...
          jd2[i] = day+daystart
          local = (zone2[i]*100)+time2[i]
          if local lt 0 then jd2[i] = jd2[i] - 1 ; account for the UTC of the overpass
  endfor
...
; ####################################################
; OLD PRINT STATEMENT 10/20/2009
...
;       bare[j],area,bmass,CO2,CO,CH4,H2,NOX,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,Hg,Factortrop[j]

; NEW PRINT STATEMENT 04/13/2011
;      printf, 6, 'longi,lat,day,genLC,CO2,CO,CH4,H2,NOx,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,Hg'
;      form = '(D20.10,",",D20.10,",",(2(I10,",")),18(D25.10,","))'
printf,6,format = form, lon[j],lat[j],jd[j],genveg, $
       CO2,CO,CH4,H2,NOX,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,Hg
