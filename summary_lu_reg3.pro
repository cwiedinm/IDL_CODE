...
; Edited on March 13 to have different regions
...
; Set output file
outfile2 = path + 'Emis_LU_Hg_summ_'+year+'_regions.txt'
...
if region eq 1 then begin ; PNW
    us = where(state eq 'Washington' or state eq 'Oregon' or state eq 'Idaho')
endif
if region eq 2 then begin ; CA
    us = where(state eq 'California')
endif
if region eq 3 then begin   ; Rocky Mountain States
    us = where(state eq 'Colorado' or state eq 'Montana' or state eq 'Wyoming')
endif
if region eq 4 then begin   ; Desert West
    us = where(state eq 'Arizona' or state eq 'Utah' or state eq 'Nevada')
endif
if region eq 5 then begin   ; Southeast
    us = where(state eq 'Alabama' or state eq 'Florida' or state eq 'Georgia' $
       or state eq 'Louisiana' or state eq 'Mississippi' or state eq 'South Carolina')
endif
