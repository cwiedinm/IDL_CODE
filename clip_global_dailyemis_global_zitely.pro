...
pro CLIP_global_dailyemis_global_ZITELY
  ...
  if j eq 10 then begin ; 2012
    year = 2012
      infile = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\FEBRUARY2013\GLOBAL_2012_02272013.txt'
       output1 = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\OCTOBER2013\ZITELY\MEX_MAR-JULY_2012.txt'
  endif
   if j eq 11 then begin ; 2013
    year = 2013
      infile = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\JULY2013\GLOBAL_JAN-JUL2013_07312013.txt'
       output1 = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\OCTOBER2013\ZITELY\MEX_MAR-JULY_2013.xt'
  endif
    
openw, 1, output1
printf, 1, 'day,CO,PM2.5, Area, BiomassBurned'
form = '(I6,4(",",D25.10))'
...
;  nmhc[k] = data1[21]
  ...
  pm10[k] = data1[28]
  area[k] = data1[10] ; m2
  bmass[k] = data1[11] ; kg dm/m2
  bburn[k] = area[k] * bmass[k] ; kg dm
endfor
close,ilun
free_lun,ilun
...
for i = daymin,daymax do begin ; Sum all emissions over each day 
   ...
   if today[0] gt -1 then begin   
      COtot = total(CO[today]) ; kg
      PM25tot = total(PM25[today]) ;kg
      Areatot = total(area[today]) ; m2
      bmasstot = total(bburn[today]) ; kg
      printf, 1, format = form, i, COtot, PM25tot, Areatot, Bmasstot
  endif else begin
       printf, 1, format = form, i, 0.0, 0.0,0.0,0.0
  endelse
      COtot = 0.0
      PM25tot = 0.0
      AreaTot = 0.0
