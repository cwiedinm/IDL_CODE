...
;   
; DECEMBER 10-11, 2012
; - Rewrote this code to use the FCCS land cover and fuel loadings (Still using the MODIS Rapid Fire Fire Counts)
; - 
; 
; *********************************************************************************************

pro global_fire_fccs
...
; ****************************************************************************
;;   READ IN FCCS FUEL LOADING FILE
...
; LCT GenVegType  GenVegDescript  CO2 CO  CH4 NMOC  H2  NOXasNO SO2 PM25  TPM TPC OC  BC  NH3 NO  NO2 NMHC  PM10
       ...
       ; Emission Factors have the units g/kg
...
; OUTPUT FILE
; ****************************************************************************
        outfile = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\DECEMBER2012\CONUS_2012_FCCS_12112012_chk.txt'
     openw, 6, outfile
     print, 'opened output file: ', outfile
     printf, 6, 'longi,lat,day,TIME,fccs,lct,genLC,pcttree,pctherb,area,bmass,CO2,CO,CH4,H2,NOx,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,FACTOR'
     form = '(D20.10,",",D20.10,",",(5(I10,",")),4(D20.10,","),17(D25.10,","),F6.3)'
 
    ; Print an output log file
;    logfile = 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\NOVEMBER2012\LOG_GLOBAL_JAN-NOV2012_11082012.txt'
;       openw, 9, logfile
...
; ****************************************************************************
  ...
  infile = 'E:\Data2\wildfire\COLORADO\Fires2012\FCCS\FIre_Points_RSAC_FCCS.csv' ; RAN ON 12/11/2012
...
; 1  2     3     4     5    6    7   8   9     
;LAT,LONG,DATE,JULIAN,GMT,SPIX,TPIX,CONF,FCCS
;49.45300000000,-96.13500000000,5/14/2012,135,1905,1.1,1.0,62,-9999

        lat1 = map.field1
        lon1 = map.field2
        spix1 = map.field6
        tpix1 = map.field7 ; TRACK, added on 02.23.2009
        date1 = map.field3
        time1 = map.field5 ; Added 02.24.2009
        CONF = map.field8    ; ADDED 08/25/08
        fccs1 = map.field9
...
; First, get rid of all fires with confidence less than 20%
		goodfires= where(conf gt 20 and fccs1 gt -1)
...
lct = intarr(ngoodfires)
tree = fltarr(ngoodfires)
herb = fltarr(ngoodfires)
...
; 1) Identify which FCCS code we have
  fccsnow = where(fccs[j] eq fccscode)
  lct[j] = lctfccs[fccsnow]
  tree[j] = perctree[fccsnow]
  herb[j] = percherb[fccsnow]
...
; ASSIGN CF VALUES (Combustion Factors)
    if (tree[j] gt 0.60) then begin      ;FOREST
    ...
    if (tree[j] gt 0.40) and (tree[j] le 0.60) then begin   ;WOODLAND
    ...
    If (tree[j] le 0.40) then begin       ;GRASSLAND
...
; Woodlands
if (tree[j] gt 0.40) and (tree[j] le 0.60) then begin
       Bmass = (pctherb*herbbm*CF3) + (pcttree*(herbbm*CF3+coarsebm*CF1))
endif
; Forests
if tree[j] gt 0.60 then begin
       Bmass = (pctherb*herbbm*CF3) + (pcttree*(herbbm*CF3+coarsebm*CF1))
endif
...
; 'longi,lat,day,TIME,fccs,lct,genLC,pcttree,pctherb,area,bmass,CO2,CO,CH4,H2,NOx,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,FACTOR'
printf,6,format = form, lon[j],lat[j],jd[j],time[j],fccs[j],lct[j],genveg,tree[j],herb[j], $
       area,bmass,CO2,CO,CH4,H2,NOX,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,Factortrop[j]
