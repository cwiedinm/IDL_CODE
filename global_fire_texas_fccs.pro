...
; - running 2013
; JUNE 25, 2013
;   - changed from global_fire.pro to global_fire_Texas.pro
;   - edited to readin in file for Texas sensitivities
;   - 2008 
;   
; OCTOBER 29, 2013
; - trying to rerun the NEW EMIS run from JUNE 2013
; 
; OCTOBER 30, 2013
; - Rerunning all simulations
;*******   
; - Resaved as global_fire_texas_FCCS  
; 
; NOVEMBER 18, 2013
; - Found mistake. The input FCCS file has the pcttree and pct herb in fractions. 
; This needs to be considered when calculating the fuel combustion. --This wanted %!! 
; So until now, everything is treated like grasslands with no woodybiomass urning!!!
; Editing code now. 
; Now,when the fractions of tree and herb are read in from the FCCS file, they are converted right away to %
; *********************************************************************************************

pro global_fire_TEXAS_FCCS
...
close, /all
 ...
 confmin = 20 ; SET MINIMUM CONFIDENCE LEVEL ; this was changed at some point in JUNE!!
 
 
 ; Set up a flag to use LCT and FCCS or just FCCS (10/30/2013)
 Allfccs = 0 ; 0 = All FCCS; 1 = use LCT and FCCS
...
;    fuelin = 'F:\Data2\wildfire\GLOBAL_MOZART\MODEL\INPUT\Model_inputs\fuelloads_new10192009.csv'; Updated 10/19/2009
    ;fuelin = 'E:\Data2\wildfire\GLOBAL_MOZART\MODEL\INPUT\Model_inputs\Fuel_LOADS_NEW012010.csv' ; Updated 01/18/2010
     ; fuelin = 'E:\Data2\wildfire\COLORADO\Fires2012\FCCS\FCCSloadings-CONUS-AK-mar2011.csv'
     fuelin = 'E:\Data2\wildfire\COLORADO\Fires2012\FCCS\FCCSloadings-CONUS-AK-oct2013.csv' 
     infuel=ascii_template(fuelin)
     fuel=read_ascii(fuelin, template=infuel)
     
;   Set up fuel arrays
;     1         2            3            4        5      6             7            8               9
; fuelbedID,fuelbedname,LCT_equivalent,totalTrees,shrub,nonwoody-live,TOTAL (g/m2),Fraction_forest,Fraction_Herv

       fccscode  = fuel.field1
       lctfccs = fuel.field3  ;LCT code to go with FCCS code
       treefuel = fuel.field4  ;Total Tree Fuel 
       shrubfuel = fuel.field5  ;shrub fuel load
       herbfuel = fuel.field6 ;herbaceous Fuel load
       totfuel = fuel.field7 ; total fuel load
       perctree = fuel.field8  ;percent tree : These numbers are really FRACTIONS
       percherb = fuel.field9 ; percent herbaceous
       ; NOTE: Fuels read in have units of g/m2 DM

; NOVEMBER 18, 2013: Converting the PERCTREE and PERCHERB to PERCENTAGES (11/18/2013)
  perctree= perctree*100.
  percherb = percherb*100.
...
;   READ IN EMISSION FACTOR FILE
     ...
     ;emisin = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_INPUTS\NEW_EFS_06242013.csv'
     ;emisin = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_INPUTS\NEW_EFS_UPPER_06242013.csv'
     ;emisin = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_INPUTS\NEW_EFS_LOWER_06242013.csv'
     ;emisin = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_INPUTS\NEW_EFS_10282013.csv' ; Added 10/30/2013
     ;emisin = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_INPUTS\NEW_EFS_UPPER_10282013.csv' ; Added 10/30/2013
     ; emisin = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_INPUTS\NEW_EFS_LOWER_10282013.csv'
...
; LCT Type  Descript  CO2 CO  CH4 NMOC  H2  NOXasNO SO2 PM25  TPM TPC OC  BC  NH3 NO  NO2 NMHC  PM10
; LCT,Type, Descript, CO2,CO, CH4,NMOC, H2, NOXasNO,SO2,PM25, TPM,TPC,OC, BC, NH3,NO, NO2,NMHC,PM10
; LCT,ype,  Descript, CO2,CO, CH4,NMOC,H2,NOXasNO,  SO2,PM25, TPM,TPC,OC, BC, NH3,NO,NO2,NMHC,PM10
...
; OUTPUT FILE
; ****************************************************************************
    outfile = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_OUTPUT\CHECK10302013\FINN_FCCS_11182013.txt'

     openw, 6, outfile
     print, 'opened output file: ', outfile
     printf, 6, 'longi,lat,day,TIME,lct,genLC,globreg,pcttree,pctherb,pctbare,area,bmass,CO2,CO,CH4,H2,NOx,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,FACTOR,state,FCCS'
     form = '(D20.10,",",D20.10,",",(8(I10,",")),2(D20.10,","),17(D25.10,","),F6.3,",",A25,",",I6)'
 
    ; Print an output log file
    logfile = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_OUTPUT\CHECK10302013\LOG_FINN_FCCS_11182013.txt'
       openw, 9, logfile
...
;     inpath='D:\Data2\wildfire\AMAZE\MODIS_RR\global_april_aug2008\global_model\'
  ...
  ;infile = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODISFRMS\Testin.csv'  ;RAN ON 06/25/2013
...
; LATITUDE,LONGITUDE,BRIGHTNESS,SCAN,TRACK,ACQ_DATE,ACQ_TIME,CONFIDENCE,LCT,TREE,HERB,BARE,REGION,Region_num,LANDFIR,ADMIN_NAME,CNTRY_NAME,GLOBCOVER,FCCS
  ...
  ; ADDED 10/30/2013 
  if globcover eq 1 then lct1 = map.field18
  if globcover eq 0 then lct1 = map.field09
...
; Added on 02.23.2009
; ***EDIT % CONFIDENCE!!!! 
; For October 2011 runs- set to -1!!!
		goodfires= where(conf gt confmin)
...
Print, 'The percent removed for conf < 20 = ', ((numorig - ngoodfires)/numorig)*100.
...
; ; DEC. 07, 2009: Date format = '2003-01-12
; 2008 files: '2008-01-30'
; 01/26/2010: 2006 file: 2006-01-14
; 04/30/2010: 2009 file: 2009-11-03
; 08/24/2010: 2005 file: 2005-01-20
; 03/25/2011: 2008 file: 2008-08-02
;  2002 file: 2002-09-05 00:00:00
 
; Global 2011 file: 2011-10-24
...
; ***********************************************
; 10/30/2013 Copied this part from other FCCS Code
; GET The LCT code for each FCCS Code
;       fccscode  = fuel.field1
;       lctfccs = fuel.field3  ;LCT code to go with FCCS code
;       treefuel = fuel.field4  ;Total Tree Fuel 
;       shrubfuel = fuel.field5  ;shrub fuel load
;       herbfuel = fuel.field6 ;herbaceous Fuel load
;       totfuel = fuel.field7 ; total fuel load
;       perctree = fuel.field8  ;percent tree
;       percherb = fuel.field9 ; percent herbaceous

; 1) Identify which FCCS code we have ; EDITED 11/1/2031
  if fccs[j] lt 0 then begin
    print, 'fccs = ', fccs[j], ' so I am skipping'
    goto, skipfire
  endif
  
  fccsnow = where(fccs[j] eq fccscode)
  
; These should be percentages (11/18/2013)
  tree[j] = perctree[fccsnow] ; In Input SECTION -- already Converted the fractions to percentages (11/18/2013)
  herb[j] = percherb[fccsnow] ; In Input SECTION -- already Converted the fractions to percentages (11/18/2013)
  ; These values are in g/m2
  bmass = totfuel[fccsnow]
  herbbm = herbfuel[fccsnow]+shrubfuel[fccsnow]
  coarsebm = totfuel[fccsnow] - herbbm


; Added 10/30/2013: Set the LCT based on the FCCS category? 
  if allfccs eq 0 then lct[j] = lctfccs[fccsnow]
;**************************************************************************
...
;    7 = temperate evergreen forest (ADDED JUNE 25, 2013)
;    
...
; THIS IS FROM THE ORIGINAL CODE (10/30/2013)
...
; THIS IS FROM THE NEW CODE with LCT = 7 (From JUNE 2013) (10/30/2013)
; 5) Forests (based on latitude)
;    if lct[j] eq 2 then genveg = 3 ; Tropical Forest
;    if lct[j] eq 4 then genveg = 4 ; Temperate Forest
;    if lct[j] eq 1 or lct[j] eq 3 then begin  ; Needleleaf forests
;       if lat[j] gt 50. then genveg = 5 else genveg = 7   ; 10/19/2009: Changed the latitude border to 50degrees N (from 60 before) and none in S. Hemisphere
;       goto, endveg                                      ; Assign Boreal for Lat > 50; Temperate for all else
;    endif                                                  ; THIS GENVEG = 7 was added in JUNE 2013
;    
;    if lct[j] eq 5 then begin ; Mixed Forest, Assign Fuel Load by Latitude
;       if lat[j] gt 50 then begin  ; 10/19/2009: Changed the latitude border to 50degrees N (from 60 before) and none in S. Hemisphere
;         genveg = 5
...
;    9 = croplands
;    0 = no vegetation (should have been removed by now- but just in case...)
;    7 = needleaf temperate forest ; ADDED JUNE 25, 2013
...
; 
;*************
; FOR SIMULATION ON 10/30/2013, this part of the code is commented out, since using the fuel loadings from FCCS)
;*************
...
;    if genveg eq 5 then bmass1 = bffuel[reg] ; boreal forests
;    if genveg eq 7 then bmass1 = tefuel[reg] ; temperate forest
...
; Mtree is the Woody biomass burned
 ...
 ; IN THE FCCS RUN- The Herb and TREE in the input file are already in fractions    
    pctherb = herb[j]/100.
    pcttree = tree[j]/100.
...
; 
; ORIGINAL EMISSION FACTOR ASSIGNMENTS: (10/30/2013)
...
;
; NEW EMISSION FACTOR ASSIGNMENTS
; JUNE 25, 2013: Changed the emission factors for evergreen temperate (10/30/2013)
;    if lct[j] eq 1 then begin
;        if lat[j] gt 50. then index = 0 else index = 14
;    endif
;    if lct[j] eq 2 then index = 1 
;    if lct[j] eq 3 then begin
;        if lat[j] gt 50. then index = 0 else index = 14
;    endif
;    if lct[j] eq 4 then index = 3
;    if lct[j] eq 5 then index = 4
...
printf,6,format = form, lon[j],lat[j],jd[j],time[j],lct[j],genveg,globreg[j],tree[j],herb[j], $
       bare[j],area,bmass,CO2,CO,CH4,H2,NOX,NO,NO2,NH3,SO2,NMHC,NMOC,PM25,TPM,OC,BC,TPC,PM10,Factortrop[j], state[j], Fccs[j]
...
printf, 9, 'The minimum confidence was set to: ', confmin
printf, 9, 'GLOBCOVER was set to: ', GLOBCOVER
printf, 9, ' '
printf, 9, 'ALL FCCS was set to: ', allfccs
printf, 9, ' '
