...
; USER interface questions to define run
 ...
 ;read, 'What extension do you want on the files?,'ext
 ;year = '2007'
 ;ext = 'Sept_Dec_NA_RR'
	ext='NEWCONF_09052008'	; used this for runs on Sept. 05, 2008
...
; 	DATE2,TIME,LAT,LONG,FRP,SPIX,CONF,GLC,PCT_TREE,PCT_HERB,PCT_BARE,ADMIN_NAME,CNTRY_NAME,ZONE,
	...
	; Using this file for input on Oct. 26, 2007
	;infile = 'D:\Data2\wildfire\EPA_EI\Brad_files\Ca_2007_7day_10262007\modis_fire_last7_2007_299_conus_shapefile\Model_in.csv'

	; FEB. 07, 2008: This infile is for North American RR fires from Sept.- December 2007
;	infile = 'D:\Data2\wildfire\Gabi_Pfister\California2007\MODIS_FIRES_FEB2008\INPUTS\NA_Sept-Dec_2007_ModelIn.csv'
;
...
; OUTPUT file pathway
    ...
    path2 = 'D:\Data2\wildfire\Gabi_Pfister\California2007\MODIS_FIRES_FEB2008\OUTPUTS\'

; Set up output file (comma delimited text file)

     outfile = 'Modelout_2007_'+ext+'.txt'
...
; DATE2,TIME,LAT,LONG,FRP,SPIX,CONF,GLC,PCT_TREE,PCT_HERB,PCT_BARE,ADMIN_NAME,CNTRY_NAME,ZONE,
    ...
    lon = map.field02
	spix = map.field03
    date2 = map.field04	; This is text date (e.g., 12/24/2007)
    time = map.field05
	conf = map.field06	;NOT CONFIDENCE IN THE 7-day file on Oct. 26, 2007
	glc = map.field07
 	bare = map.field08*1.0
	herb = map.field09*1.0
    tree = map.field10*1.0
    zone = map.field11
    state = map.field12
	cntry = map.field13
...
; Calculate JD for each fire (This is new, September 17, 2007)
	...
	; NOTE: THIS IS SET UP FOR 2006 ONLY... need to change this if change the year...
		; 10/26/2007: Changed the Julday to 2006 for 2007 fires
	jd[j] = Julday(parts[0],parts[1],parts[2]) - Julday(12,31,2006)
...
; 	10/26/2007: commented this out for the 7-day continental US - doesn't include confidence.
