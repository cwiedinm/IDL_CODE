 ...
 ; DECEMBER 10, 2008: RX2
 ; 		- using original default emission factors, but editing the Combustion Efficiencies
 ; ******************************************************************************************************************

 pro Fire_Emis_v4_RX2
...
;
; Input file path
	pathy = 'D:\Data2\wildfire\AGU2008\INPUT_FILES\'
; 	pathy = 'D:\Data2\wildfire\ ; for the laptop

; DECLARE INPUT FILE NAME
     infile = 'D:\Data2\wildfire\AGU2008\INPUT_FILES\Fire_input_file_2007.txt'
    ; infile = 'D:\Data2\wildfire\AGU2008\MODEL_OUTPUT\CHECK\2006185_modelinputs_CAonly.csv'
    ; infile = 'C:\WORK\FIRE\Fire_EMIS_122008\INPUT_FILES\Fire_input_file_2002.txt' ; For the laptop
; ******************************************************************
; **************************************************************************************
;                  OUTPUT FILES
; ******************************************************************
; OUTPUT file pathway
    ...
    path2 = 'D:\Data2\wildfire\AGU2008\MODEL_OUTPUT\'
...
; Set up output file (comma delimited text file)
     ...
     outfile = 'Modelout_test_'+ext+'.txt'
...
; 	and were not significantly different. Will update again before this version is finalized
 	glcfactor = path4+'NEW_EFs_DEC2008a.csv'
;	glcfactor = path4+'RX1.csv' ; RX1 emission factors.
...
; 	weststatesfile = 'C:\WORK\FIRE\Fire_EMIS_122008\default_infiles\western_states.csv' ; for the laptop
	weststatesfile = 'D:\Data2\wildfire\EPA_EI\MODEL\default_infiles\western_states.csv'
...
; REMOVE ALL FIRES WITH CONFIDENCE < 20
    ...
    FRP = map.field09[okconf]			; ADDED on Nov. 03, 2008 for inclusion of new fire counts
    date2 = map.field05[okconf]			; This is text date (e.g., 12/24/2007)
    time = map.field07[okconf]		; UTC time
	conf = map.field10[okconf]
	glc = map.field11[okconf]
 	bare = map.field14[okconf]*1.0
	herb = map.field13[okconf]*1.0
    tree = map.field12[okconf]*1.0
    zone = map.field17[okconf]
    state = map.field15[okconf]
	cntry = map.field16[okconf]
	fed = map.field18[okconf]
; *****************************************************************************************
...
; EDITED FOR RX2 on DECEMBER 10, 2008
; For forests and woodlands, edited the CE so that the 75% of the herbaceous fuels and
; 		5% of the woody fuels burn.
; 		Left grasslands as they are.

    if (tree[j] gt 60) then begin      ;FOREST
    ; Values from Table 3 Ito and Penner [2004]
        CF1 = 0.05          ; Live Woody
        CF3 = 0.75          ; Leafy Biomass
        CF4 = 0.75          ; Herbaceous Biomass
        CF5 = 0.75          ; Litter Biomass
        CF6 = 0.05          ; Dead woody
    endif
...
;
 ;form = '(I8,",",D20.10,",",D20.10,",",(5(I15,",")),3(D20.10,","),12(D25.10,","),3(A25,","))'
;print, 'print1'
            printf,6,format=form,j,lon[j],lat[j],jd[j],glc[j],tree[j],herb[j],bare[j],frp[j],area,bmass, $
