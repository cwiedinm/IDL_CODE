...
pro process_TEXAS_FIRE_EMISS_SCENARIOS_domain
t0 = systime(1) ;Procedure start time in seconds
close, /all

year = 2008

for j = 0,6 do begin ; do same processing for every file
  close, /all
  inpath = 'E:\Data2\wildfire\TEXAS\FIRE_2008\MODEL_OUTPUT\'
  if j eq 0 then begin ; 
      infile = inpath + 'FINNv1_TX_default_06252013.txt'
      output1 = inpath + 'FINNv1_TX_default_06252013_SUMMARY.txt'
  endif
...
printf, 1, 'state, month, npoints, PM2.5, CO'
form = '(A25,",",I4,",",I8,2(",",D25.10))'
...
for i = 0,11 do begin
 ...
 ;        nstates = n_elements(statename)
        ; for k = 0,nstates-1 do begin  ; Do loop over state names
              COtotal = 0.0
              PMtotal = 0.0              
              firenow = where(lati[monthnow] ge 23. and lati[monthnow] le 51. and longi[monthnow] ge -126. and longi[monthnow] le -90.)
              ...
              ;
              ;printf, 1, 'state, month, npoints, PM2.5, CO'
              ;form = '(A25,",",I4,2(",",D25.10))'
              printf, 1, format = form, 'Domain' ,month,n_elements(firenow),PMtotal,COtotal
              skip21:
