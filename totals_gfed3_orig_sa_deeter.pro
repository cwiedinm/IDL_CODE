...
pro totals_gfed3_orig_SA_deeter
...
openw, 1, 'E:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\APRIL2013\DEETER\GFEDv3_monthly_Totals_Amazon_Deeter2.txt'
printf,1,'GFED-v3.1 '
 ...
 latw = coslat/total(coslat)
  ...
  for iyr=0,nyrs-1 do begin
   for imon = 0,11 do begin
     ...
     spectot[iyr,imon] = totemis *1.e-9 *12.  ;Gg/yr
   endfor
   printf,1,format='(a15,",",i6,12(",",f10.4))',spec,years[iyr],spectot[iyr,*]
  endfor
  ncdf_close,nc_id
