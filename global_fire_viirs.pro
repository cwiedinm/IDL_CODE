...
;   * For fires with bad VCF - changed teh assignment to forest, shrub, or grass
...
; OUTPUT FILE
; ****************************************************************************
     outfile = 'E:\Data2\wildfire\INDONESIA\OUTPUT\testout05052014.txt'
    ...
    logfile = 'E:\Data2\wildfire\INDONESIA\OUTPUT\LOG_testout05052014.txt'
       openw, 9, logfile
...
; ****************************************************************************
  ...
  infile = 'E:\Data2\wildfire\INDONESIA\INPUT\input_for_finn_050514.csv' 
...
if globcover eq 0 then begin
  grass = map.field10*1.0
  shrub = map.field11*1.0
  tropfor = map.field12*1.0
  tempfor = map.field13*1.0
  borfor = map.field14*1.0
  crop = map.field15*1.0
endif

if globcover eq 0 then begin
  grass = map.field16*1.0
  shrub = map.field17*1.0
  tropfor = map.field18*1.0
  tempfor = map.field19*1.0
...
;    reassign cover values based on LCT assignment
       if totcov[j] ge 240. or totcov[j] lt 1. or bare[j] eq 100 then begin ; this also include where VCF see water (values = 253)
          allbare = allbare+1
         if (tropfor[j]+tempfor[j]+borfor[j]) ge 60. then begin    ; Assign forest to the pixel
          tree[j] = 60.
          herb[j] = 40.
          bare[j] = 0.
         endif
         if shrub[j] ge 60.then begin    ; Assign woody savanna to the pixel
          tree[j] = 50.
          herb[j] = 50.
          bare[j] = 0.
         endif
         if grass[j]+crop[j] ge 60. or (shrub[j] lt 40. AND  (tropfor[j]+tempfor[j]+borfor[j]) lt 40.) then begin  ; Assign grassland to the pixel
          tree[j] = 20.
          herb[j] = 80.
          bare[j] = 0.
         endif
...
; CALCULATE EMISSIONS kg/day
       CO2 = area/1000.*(CO2EF[8]*bmass1+CO2EF[6]*bmass2+CO2EF[1]*bmass3+CO2EF[3]*bmass4+CO2EF[2]*bmass5+CO2EF[11]*bmass6)
       CO = area/1000.*(COEF[8]*bmass1+COEF[6]*bmass2+COEF[1]*bmass3+COEF[3]*bmass4+COEF[2]*bmass5+COEF[11]*bmass6)
       CH4 = area/1000.*(CH4EF[8]*bmass1+CH4EF[6]*bmass2+CH4EF[1]*bmass3+CH4EF[3]*bmass4+CH4EF[2]*bmass5+CH4EF[11]*bmass6)
       NMOC = area/1000.*(NMOCEF[8]*bmass1+NMOCEF[6]*bmass2+NMOCEF[1]*bmass3+NMOCEF[3]*bmass4+NMOCEF[2]*bmass5+NMOCEF[11]*bmass6)
       NMHC = area/1000.*(NMHCEF[8]*bmass1+NMHCEF[6]*bmass2+NMHCEF[1]*bmass3+NMHCEF[3]*bmass4+NMHCEF[2]*bmass5+NMHCEF[11]*bmass6)
       H2 = area/1000.*(H2EF[8]*bmass1+H2EF[6]*bmass2+H2EF[1]*bmass3+H2EF[3]*bmass4+H2EF[2]*bmass5+H2EF[11]*bmass6)
       
       NOX = area/1000.*(NOxEF[8]*bmass1+NOxEF[6]*bmass2+NOxEF[1]*bmass3+NOxEF[3]*bmass4+NOxEF[2]*bmass5+NOxEF[11]*bmass6)
       NO = area/1000.*(NOEF[8]*bmass1+NOEF[6]*bmass2+NOEF[1]*bmass3+NOEF[3]*bmass4+NOEF[2]*bmass5+NOEF[11]*bmass6)
       NO2 = area/1000.*(NO2EF[8]*bmass1+NO2EF[6]*bmass2+NO2EF[1]*bmass3+NO2EF[3]*bmass4+NO2EF[2]*bmass5+NO2EF[11]*bmass6)
       SO2 = area/1000.*(SO2EF[8]*bmass1+SO2EF[6]*bmass2+SO2EF[1]*bmass3+SO2EF[3]*bmass4+SO2EF[2]*bmass5+SO2EF[11]*bmass6)
       PM25 = area/1000.*(PM25EF[8]*bmass1+PM25EF[6]*bmass2+PM25EF[1]*bmass3+PM25EF[3]*bmass4+PM25EF[2]*bmass5+PM25EF[11]*bmass6)
       TPM = area/1000.*(TPMEF[8]*bmass1+TPMEF[6]*bmass2+TPMEF[1]*bmass3+TPMEF[3]*bmass4+TPMEF[2]*bmass5+TPMEF[11]*bmass6)
       TPC = area/1000.*(TCEF[8]*bmass1+TCEF[6]*bmass2+TCEF[1]*bmass3+TCEF[3]*bmass4+TCEF[2]*bmass5+TCEF[11]*bmass6)
       OC = area/1000.*(OCEF[8]*bmass1+OCEF[6]*bmass2+OCEF[1]*bmass3+OCEF[3]*bmass4+OCEF[2]*bmass5+OCEF[11]*bmass6)
       BC = area/1000.*(BCEF[8]*bmass1+BCEF[6]*bmass2+BCEF[1]*bmass3+BCEF[3]*bmass4+BCEF[2]*bmass5+BCEF[11]*bmass6)
       NH3 = area/1000.*(NH3EF[8]*bmass1+NH3EF[6]*bmass2+NH3EF[1]*bmass3+NH3EF[3]*bmass4+NH3EF[2]*bmass5+NH3EF[11]*bmass6)
       PM10 = area/1000.*(PM10EF[8]*bmass1+PM10EF[6]*bmass2+PM10EF[1]*bmass3+PM10EF[3]*bmass4+PM10EF[2]*bmass5+PM10EF[11]*bmass6)

bmassburn = area*(bmass1+bmass2+bmass3+bmass4+bmass5+bmass6) ; kg burned
...
printf,6,format = form, lon[j],lat[j],time[j],jd[j],peat[j],grass[j],shrub[j],tropfor[j],tempfor[j],borfor[j],crop[j],tree[j],herb[j],bare[j], $
