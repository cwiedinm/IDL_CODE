...
; Set Files
    ...
    outfile = outpath+'fire2004_allvoc.nc'
    checkfile = outpath+'check_firetots2004_12132004.txt'
...
; Find the files in each month
       jan = where(fire.field04 ge 1 and fire.field04 lt 32)
       feb = where(fire.field04 ge 32 and fire.field04 lt 60)
       mar = where(fire.field04 ge 60 and fire.field04 lt 91)
       apr = where(fire.field04 ge 91 and fire.field04 lt 121)
       may = where(fire.field04 ge 121 and fire.field04 lt 152)
       jun = where(fire.field04 ge 152 and fire.field04 lt 182)
       jul = where(fire.field04 ge 182 and fire.field04 lt 213)
       aug = where(fire.field04 ge 213 and fire.field04 lt 244)
       sep = where(fire.field04 ge 244 and fire.field04 lt 274)
;       oct = where(fire.field04 ge 274 and fire.field04 lt 305)
;       nov = where(fire.field04 ge 305 and fire.field04 lt 335)
;       dec = where(fire.field04 ge 335 and fire.field04 lt 366)


    print, 'the number of fires in January is:', n_elements(jan)
    print, 'the number of fires in Feb. is:', n_elements(feb)
    print, 'the number of fires in March is:', n_elements(mar)
    print, 'the number of fires in April is:', n_elements(apr)
    print, 'the number of fires in May is:', n_elements(may)
    print, 'the number of fires in June is:', n_elements(jun)
    print, 'the number of fires in July is:', n_elements(jul)
    print, 'the number of fires in Aug. is:', n_elements(aug)
    print, 'the number of fires in Sept. is:', n_elements(sep)
;    print, 'the number of fires in Oct. is:', n_elements(oct)
;    print, 'the number of fires in Nov. is:', n_elements(nov)
;    print, 'the number of fires in Dec. is:', n_elements(dec)
;    print, 'The total number of fires in 2004 is:', n_elements(fire.field04)
...
; Set up output Arrays
    CO2_emis = fltarr(110,61,9)
    CO_emis = fltarr(110,61,9)
    NOX_emis = fltarr(110,61,9)
    VOC_emis = fltarr(110,61,9)
    SO2_emis = fltarr(110,61,9)
    NH3_emis = fltarr(110,61,9)
    PM25_emis = fltarr(110,61,9)
    PM10_emis = fltarr(110,61,9)
    CH4_emis = fltarr(110,61,9)

; Set up speciated VOC arrays
    C2H6emis =  fltarr(110,61,9)
    C2H4emis = fltarr(110,61,9)
    C3H8emis =  fltarr(110,61,9)
    C3H6emis =  fltarr(110,61,9)
    CH2Oemis =  fltarr(110,61,9)
    CH3OHemis =  fltarr(110,61,9)
    BIGALKemis =  fltarr(110,61,9)
    BIGENEemis =  fltarr(110,61,9)
    CH3COCH3emis =  fltarr(110,61,9)
    C2H5OHemis = fltarr(110,61,9)
    CH3CHOemis =  fltarr(110,61,9)
    MEKemis =  fltarr(110,61,9)
    TOLemis =  fltarr(110,61,9)
    NOemis = fltarr(110,61,9)
    OCemis =  fltarr(110,61,9)
    BCemis =  fltarr(110,61,9)
...
; Fill in Arrays by each month
for i = 0, 8 do begin  ; Go through every month, even though there may be no data for those months
    ; Set up with month
    if i EQ 0 then month = jan
    if i EQ 1 then month = feb
    if i EQ 2 then month = mar
    if i EQ 3 then month = apr
    if i EQ 4 then month = may
    if i EQ 5 then month = jun
    if i EQ 6 then month = jul
    if i EQ 7 then month = aug
    if i EQ 8 then month = sep
;    if i EQ 9 then month = oct
;    if i EQ 10 then month = nov
;    if i EQ 11 then month = dec

;    if month eq -1 the goto, skipmonth
