...
;   
; DECEMBER 06, 2010
;   - edited to run 2006 for Bob; Mexico only
;   - added biomass burned to output
;  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$

pro speciate_global_SAPRC99_BOB
close, /all

;****************************************************************************************************
; CONVERSION FACTOR TABLE FOR VOCs
      convert = 'F:\Data2\wildfire\EMISSIONS_PAPER2009\SPECIATION\SAPR99_VOC_Speciate_12012010.csv'
;      convert ='F:\Data2\wildfire\EMISSIONS_PAPER2009\SPECIATION\SAPR99_VOC_Speciate.csv'
...
;infile = 'F:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\SEPTEMBER2010\GLOB_JAN-NOV2009_09082010.txt'
;infile ='F:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\SEPTEMBER2010\GLOB_MAY-SEPT2007_09072010.txt' 
outfile = 'F:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\NOVEMBER2010\SPECIATE\SAPRC99\GLOB_2006_12062010_SAPRC99_MXonly.txt' 
checkfile = 'F:\Data2\wildfire\MODEL_OUTPUTS\GLOBAL\NOVEMBER2010\SPECIATE\SAPRC99\LOG\CHECK_GLOB_2006_12062010_SAPRC99_MXonly.txt'
...
printf, 5, 'DAY,TIME,GENVEG,BIOBURN,LATI,LONGI,CO2,CO,NO,NO2,SO2,NH3,CH4,VOC,ACET,ALK1,ALK2,ALK3,ALK4,ALK5,ARO1,ARO2,BALD,CCHO,CCO_OH,ETHENE,HCHO,HCN,HCOOH,HONO,ISOPRENE,MEK,MEOH,METHACRO,MGLY,MVK,OLE1,OLE2,PHEN,PROD2,RCHO,RNO3,TRP1,OC,BC,PM25,PM10'
...
for i = 0L,numfires-1 do begin
 ; ONLY OUTPUT NORTHERN HEMISPHERE
 ;   if lati[i] lt 0.0 then goto, skipfire
 
 ; FOR JEROMES MILAGRO DOMAIN Only get fires January-June and -10 to 20 and -130 to -50 longitude
 ;     if day[i] gt 181 then goto, skipfire
 ;     if lati[i] lt -10. or lati[i] gt 40. or longi[i] lt -130. or longi[i] gt -50. then goto, skipfire
; FOR JEROMES VOCALS
;     if lati[i] gt 0. then goto, skipfire

; FOR JEFF LEE'S FILES FOR ASIA in NOVEMBER 2009
;  if day[i] lt 244 then goto, skipfire ; only output SEPTEMBER - NOVEMBER
;  if lati[i] lt 0. or lati[i] gt 62. or longi[i] lt 60. or longi[i] gt 175. then goto, skipfire

; For Gail Tonnesson's Model domain
; Only North and Central America
;  if lati[i] lt 12. or longi[i] gt -52. or longi[i] lt -170. then goto, skipfire 
 
; For Oivind's domain over Greece
;  if lati[i] lt 20. or lati[i] gt 50. or longi[i] gt 39. or longi[i] lt -5. then goto, skipfire 

; For Bob's file (for 2006 in Mexico): 
   if lati[i] lt 12. or lati[i] gt 33. or longi[i] lt -119. or longi[i] gt -86.then goto, skipfire 
...
;GLYemis[i] = VOC[i]*convert2SAPR99[12]
...
;                         DAY,   TIME,   GENVEG,   BIOBURN, LATI,   LONGI,   CO2,       CO,       NO,       NO2,        SO2,       NH3,       CH4,       VOC, 
printf, 5, format = form, day[i],time[i],genveg[i],bmassburn,lati[i],longi[i],CO2emis[i],COemis[i],NOemis[i],NO2emis[i], SO2emis[i],NH3emis[i],CH4emis[i], VOCemis[i],$
